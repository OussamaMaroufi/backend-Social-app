...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        self.userRooms = await database_sync_to_async(
            list
        )(ChatRoom.objects.filter(member=self.userId))   # when connection established the user added to group layer of online user groups
    ...
    async def receive(self, text_data):
        #Here to receive data form frontend and brodcast it throught the group 
        print(text_data)
        ...
        print(chatMessage)
        await self.channel_layer.group_send(     #message brodcast to room id Here !! 
            roomId,
            {
                'type': 'chat_message',
                'message': chatMessage
